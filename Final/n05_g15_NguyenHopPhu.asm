...
while:
	...
	beq $t1, $s5, operator # '/'
	nop
	beq $t1,37,operator # 37-ascii dau %
	...
	jal check_number   #neu la so -> v0 = 1; kphai la so -> 0
	beq $v0, 1, n_operator
	nop
...
xuli2: 
	...
	beq $t2, '(', pushToStack
	nop
	# check t1 precedence, ktra bac cua phan tu dang xet
	beq $t1, $s2, t1to1
	nop
	beq $t1, $s3, t1to1
	nop
...
t1to1:
	...
	# check t2 precedence, kiem tra bac cua dinh stack
check_t2:
...
# push to stack
	la $t6, stack
	add $t6, $t6, $s7
	sb $zero, 0($t6)   #pop dinh stack, dinh stack = 0
